...
class Snowball(bs.Actor):
    ...
    def __init__(self,
                 ...
                 thrower: NewPlayerSpaz | None = None,
                 melt: bool = True,
                 bounce: bool = True,
                 explode: bool = False):
        super().__init__()
    ...
    def handlemessage(self, msg: Any) -> Any:
        if isinstance(msg, bs.DieMessage):
            if self.thrower is not None:
                active_snowballs = getattr(self.thrower, '_active_snowballs', None)
                if isinstance(active_snowballs, list):
                    try:
                        active_snowballs.remove(self)
                    except ValueError:
                        pass
                elif getattr(self.thrower, '_active_snowball', None) is self:
                    # Backward compatibility with older single-snowball state.
                    self.thrower._active_snowball = None
            if self.node:
                self.node.delete()
...
class NewPlayerSpaz(PlayerSpaz):
    ...
    def __init__(self, *args: Any, **kwds: Any):
        ...
        self._active_snowballs: list[Snowball] = []

    def on_punch_press(self) -> None:
        if not self.node or self.frozen or self.node.knockout > 0.0:
            return
        # Allow up to two live snowballs per player.
        self._active_snowballs = [
            sb for sb in self._active_snowballs
            if sb.node and sb.node.exists()
        ]
        if len(self._active_snowballs) >= 2:
        ...
        if t_ms - self.last_punch_time_ms >= self._punch_cooldown:
            ...
            self._active_snowballs.append(snowball)
