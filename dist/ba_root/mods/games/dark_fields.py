...
class DarkFieldsGame(bs.TeamGameActivity[Player, Team]):
    ...
    def on_transition_in(self) -> None:
        ...
        a = bs.newnode('locator',attrs={'shape':'box','position':(12.2,0,.1087926362),
            'color':(5,0,0),'opacity':1,'drawShadow':False,'draw_beauty':True,'additive':False,'size':[2.5,0.1,12.8]})
			
        b = bs.newnode('locator',attrs={'shape':'box','position':(-12.1,0,.1087926362),
            'color':(0,0,5),'opacity':1,'drawShadow':False,'draw_beauty':True,'additive':False,'size':[2.5,0.1,12.8]})
    ...
    def doRandomLighting(self):
        ...
        for mine in self.mines:
            if mine.node.exists():
                ...
                bs.timer(delay,bs.CallPartial(self.do_light,pos))
    ...
    def spawnMines(self):
        ...
        for h in h_range:
            for i in range(random.randint(3,4)):
                ...
                pos = (x,1,y)
                bs.timer(delay,bs.CallPartial(self.doMine,pos))
    ...
    def _on_bomb_exploded(self, bomb: Bomb, blast: Blast) -> None:
        ...
        pos = (p[0],p[1]+1,p[2])
        bs.timer(0.5,bs.CallPartial(self.doMine,pos))
