...
class VolleyBallGame(bs.TeamGameActivity[Player, Team]):
    ...
    def __init__(self, settings: dict):
        ...
        # Use defaults when loading older playlists missing some settings.
        self._score_to_win = int(settings.get('Score to Win', 1))
        self._punchie_ = bool(settings.get('Disable Punch', False))
        self._night_mode = bool(settings.get('Night Mode', False))
        self._bombies_ = bool(settings.get('Disable Bombs', False))
        self._time_limit = float(settings.get('Time Limit', 0))
        self._icy_flooor = bool(settings.get('Icy Floor', True))
        self.credit_text = bool(settings.get('Enable Bottom Credits', True))
...
try:
    bs.register_map(VolleyBallMap)
    bs.register_map(VolleyBallMapH)
except Exception:
    try:
