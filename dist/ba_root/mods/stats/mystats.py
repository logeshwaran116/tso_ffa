...
class UpdateThread(threading.Thread):
    ...
    def run(self):
        ...
        for account_id, kill_count in self._account_kills.items():
            ...
            # Accrue per-round score into conversion bank for coin economy
            try:
                score_delta = int(self.account_scores.get(account_id, 0))
                if score_delta > 0:
                    coins.accrue_score(account_id, score_delta)
            except Exception:
                pass
