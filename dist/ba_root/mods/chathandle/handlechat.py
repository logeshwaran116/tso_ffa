...
def filter_chat_message(msg, client_id):
    ...
    # Voting: end/dv/nv/sm
    if msg.lower() in ["end", "dv", "nv", "sm"] and settings["allowVotes"]:
        votingmachine.vote(acid, client_id, msg.lower())

    # Voting: playlist (vp <name>)
    if settings["allowVotes"]:
        low = msg.lower().strip()
        if low == "vp" or low.startswith("vp "):
            parts = low.split()
            if len(parts) < 2 or not parts[1]:
                bs.broadcastmessage("Usage: vp <playlist-name>; try /sap to see options", transient=True, clients=[client_id])
                return None
            pl_key = parts[1]
            playlists = settings.get("playlists", {})
            if pl_key not in playlists:
                bs.broadcastmessage("Unknown playlist. Use /sap to list.", transient=True, clients=[client_id])
                return None
            votingmachine.vote(acid, client_id, "vp", payload=pl_key)
            return None
        # Voting: if a playlist vote is active, accept '1' as a vote
        if low == "1":
            try:
                if votingmachine.is_vp_vote_active():
                    votingmachine.vote(acid, client_id, "vp")
                    return None
                else:
                    bs.broadcastmessage("No active playlist vote.", transient=True, clients=[client_id])
                    return None
            except Exception:
                bs.broadcastmessage("No active playlist vote.", transient=True, clients=[client_id])
                return None
